...
const descriptionInput = document.getElementById('description');
const shotInput = document.getElementById('foresight'); 
const levelingForm = document.getElementById('levelingForm');
const resultsForm = document.getElementById('results'); // Added ID to results form
const output = document.getElementById('output');
...
function calculateShot() {
    ...
    // Display the current state in the results form
    const resultsOutput = document.createElement('div');
    resultsOutput.innerHTML = `<p>Shot ${shotId}: "${descriptionValue}" <strong>|</strong> Initial Elevation: ${initialElevationValue} <strong>|</strong> HT: ${HeightOfTarget} <strong>|</strong> HI: ${currentHeightOfInstrument} \n</p>`;
    resultsForm.appendChild(resultsOutput);
